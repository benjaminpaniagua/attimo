...
export function Tasks() {
  ...
  const activeActivities = data.filter(activity => activity.status === 'Active');
  const inactiveActivities = data.filter(activity => activity.status === 'Inactive');
  ...
  return (
    <div className="flex w-full gap-4 md:flex-col">
      ...
      <section className="w-1/2 grid items-center content-center bg-white dark:bg-clr-dark-third p-4 text-center rounded-lg md:py-8 md:w-full">
        <Task number={activeActivities.length} description="Remaining Activities" />
      </section>
    </div>
  );
}
